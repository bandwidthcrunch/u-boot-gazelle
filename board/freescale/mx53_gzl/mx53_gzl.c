/*
 * Copyright (C) 2013 (bandwidthcrunch@gmail.com)
 * Modified liberally from various sources 
...
static inline void setup_boot_device(void)
{
	...
	printf("SOC_SMBR 0x%x\n", soc_sbmr);
	printf("BR_MEM_CTL: 0x%x\n", bt_mem_ctl);
	printf("BT_MEM_CTYPE : 0x%x\n", bt_mem_type);
	switch (bt_mem_ctl) {
	case 0x0:
		if (bt_mem_type)
...
#define USBCMD 0x140
	 ...
	 * force warm reset as cold reset
	 */
	...
	gd->bd->bi_arch_number = MACH_TYPE_MX53_GZL;	/* board id for linux */
...
int check_recovery_cmd_file(void)
{
	 ...
	 * @TODO: Need At least Two key, but in GZL board,
